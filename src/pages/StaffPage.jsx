...
import { AlertTriangle, ArrowRight, Bell, DollarSign, LockKeyhole, ShieldCheck, Map } from 'lucide-react'
...
const STAFF_LINKS = [
  ...
  },
  {
    ...
    desc: 'View and approve all pending trip requests submitted by customers awaiting agent review.',
    to: '/agent/trips',
    icon: Map,
    accent: 'bg-indigo-50 border-indigo-200 text-indigo-600',
    hover: 'hover:border-indigo-400 hover:shadow-indigo-50',
  },
  {
...
export default function StaffPage() {
  const { user } = useAuth()
  const canUseStaff = ['agent', 'admin', 'finance'].includes(user?.role)
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          ...
          <h1 className="text-4xl font-bold text-slate-900 mb-2">
            Welcome back, <span className="text-blue-600">{user?.name?.split(' ')[0] || 'Agent'}</span>
          </h1>
        ...
        {!canUseStaff && (
          <motion.div
            ...
            className="bg-amber-50 border border-amber-200 rounded-2xl p-5 mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"
          >
            ...
            return (
              ...
              >
                <Link
                  ...
                  className={`bg-white border border-slate-200 rounded-2xl p-6 h-full flex flex-col justify-between shadow-sm ${item.hover} hover:shadow-md transition-all group`}
                >
                  <div>
